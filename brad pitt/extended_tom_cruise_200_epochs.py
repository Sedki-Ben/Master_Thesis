#!/usr/bin/env python3
"""
Extended Tom Cruise: 200-Epoch Training System
==============================================

Extended training of the improved Tom Cruise CNN models with 200 epochs
to investigate if longer training leads to better convergence and performance.
Based on the tom_cruise_improved_cnn_training.py but with extended epochs.
"""
...
class ExtendedTomCruiseCNNSystem:
    """Extended Tom Cruise: 200-epoch training system"""
    ...
    def __init__(self, output_dir="extended_tom_cruise_200_epochs"):
        ...
        print(f"🎬 Extended Tom Cruise CNN System (200 Epochs) Initialized")
    ...
    # SAME IMPROVED CNN ARCHITECTURES AS TOM CRUISE
    def build_basic_cnn_improved(self, input_shape):
        ...
        model = keras.Model(inputs=csi_input, outputs=output, name='BasicCNN_Extended')
        return model
    ...
    def build_multiscale_cnn_improved(self, input_shape):
        ...
        output = layers.Dense(2, activation='linear', name='coordinates')(x)
        model = keras.Model(inputs=csi_input, outputs=output, name='MultiScaleCNN_Extended')
        return model
    ...
    def build_attention_cnn_improved(self, input_shape):
        ...
        output = layers.Dense(2, activation='linear', name='coordinates')(x)
        model = keras.Model(inputs=csi_input, outputs=output, name='AttentionCNN_Extended')
        return model
    ...
    def build_hybrid_cnn_rssi_improved(self, input_shape):
        ...
        output = layers.Dense(2, activation='linear', name='coordinates')(x)
        model = keras.Model(inputs=[csi_input, rssi_input], outputs=output, name='HybridCNN_Extended')
        return model
    ...
    def build_residual_cnn_improved(self, input_shape):
        ...
        output = layers.Dense(2, activation='linear', name='coordinates')(x)
        model = keras.Model(inputs=csi_input, outputs=output, name='ResidualCNN_Extended')
        return model
    
    def compile_and_train_model_extended(self, model, train_inputs, train_targets, val_inputs, val_targets, model_name, dataset_size):
        """Compile and train model with EXTENDED 200-epoch configuration"""
        print(f"    Training {model_name} on {dataset_size} samples (EXTENDED 200 EPOCHS)...")
        
        # Same improved configuration as Tom Cruise
        model.compile(
        ...
        print(f"    ✅ Extended epochs: 200 (up from 150)")
        
        # Model checkpoint
        checkpoint_path = self.output_dir / f"{model_name.lower()}_{dataset_size}_samples_extended.h5"
        callbacks = [
            EarlyStopping(patience=40, restore_best_weights=True, monitor='val_loss'),  # Increased patience for longer training
            ReduceLROnPlateau(factor=0.5, patience=20, min_lr=1e-7),  # Increased patience for longer training
        ...
        history = model.fit(
            ...
            epochs=200,  # EXTENDED from 150 to 200
            batch_size=16,  # Same as Tom Cruise
            callbacks=callbacks,
            verbose=1
        )
    ...
    def plot_extended_learning_curves(self, all_histories, dataset_sizes):
        """Plot learning curves for all extended models"""
        print(">>> Creating extended learning curves (200 epochs)...")
        ...
        for i, model_name in enumerate(model_names):
            ...
            ax.set_title(f'{model_name} Learning Curves (200 EPOCHS)', fontweight='bold', fontsize=12)
            ax.set_xlabel('Epoch')
            ax.set_ylabel('Loss (MSE)')
            ax.legend(fontsize=9)
            ax.grid(True, alpha=0.3)
            ax.set_xlim(0, 200)  # Set x-axis to show full 200 epochs
        
        # Hide the last subplot
        axes[5].axis('off')
        
        plt.tight_layout()
        plt.savefig(self.output_dir / 'extended_learning_curves_200_epochs.png', dpi=300, bbox_inches='tight')
        print(f"    Saved extended learning curves to {self.output_dir / 'extended_learning_curves_200_epochs.png'}")
        plt.show()
    
    def plot_accuracy_cdfs(self, all_results):
        """Plot CDFs for extended models"""
        print(">>> Creating extended accuracy CDFs...")
        ...
        for i, (threshold, name) in enumerate(zip(thresholds, threshold_names)):
            ...
            ax.set_title(f'CDF: {name} Accuracy (200 EPOCHS)', fontweight='bold')
            ax.grid(True, alpha=0.3)
            ax.legend(bbox_to_anchor=(1.05, 1), loc='upper left', fontsize=9)
            ax.set_xlim(0, min(6, max([np.max(r['errors']) for r in all_results])))
        
        plt.tight_layout()
        plt.savefig(self.output_dir / 'extended_accuracy_cdfs_200_epochs.png', dpi=300, bbox_inches='tight')
        print(f"    Saved extended CDFs to {self.output_dir / 'extended_accuracy_cdfs_200_epochs.png'}")
        plt.show()
    ...
    def save_results_summary(self, all_results):
        ...
        csv_path = self.output_dir / 'extended_200_epochs_results.csv'
        df.to_csv(csv_path, index=False)
        print(f">>> Extended 200-epoch results saved to {csv_path}")
        
        return df
    
    def run_extended_training(self):
        """Run extended 200-epoch training for all models and dataset sizes"""
        print("="*80)
        print("🎬 EXTENDED TOM CRUISE: 200-EPOCH CNN TRAINING")
        print("="*80)
        print("🚀 Extended training configuration:")
        print("   ✅ Same improved Tom Cruise settings")
        print("   ✅ Extended epochs: 150 → 200")
        print("   ✅ Extended patience: EarlyStopping=40, ReduceLR=20")
        print("   ✅ Same learning rate: 0.0002")  
        print("   ✅ Same regularization: L2=1e-4, Dropout=0.5/0.4")
        print("   ✅ Same loss function: MSE")
        print("   ✅ Same batch size: 16")
        print("="*80)
        
        dataset_sizes = [250, 500, 750]
        model_builders = {
            'BasicCNN_Extended': self.build_basic_cnn_improved,
            'MultiScaleCNN_Extended': self.build_multiscale_cnn_improved,
            'AttentionCNN_Extended': self.build_attention_cnn_improved,
            'HybridCNN_Extended': self.build_hybrid_cnn_rssi_improved,
            'ResidualCNN_Extended': self.build_residual_cnn_improved
        }
        ...
        for dataset_size in dataset_sizes:
            ...
            # NO DATA AUGMENTATION (same as Tom Cruise)
            ...
            for model_name, model_builder in model_builders.items():
                ...
                model, history = self.compile_and_train_model_extended(
        ...
        self.plot_extended_learning_curves(all_histories, dataset_sizes)
        self.plot_accuracy_cdfs(all_results)
        
        # Print final summary
        print("\n" + "="*80)
        print("🏆 EXTENDED 200-EPOCH RESULTS SUMMARY")
        print("="*80)
        
        # Sort by median error
        all_results.sort(key=lambda x: x['median_error'])
        
        print(f"{'Model':<25} {'Size':<6} {'Median (m)':<10} {'<1m (%)':<8} {'<2m (%)':<8} {'<3m (%)':<8}")
        print("-" * 80)
        
        for result in all_results:
            print(f"{result['model_name']:<25} {result['dataset_size']:<6} "
                  f"{result['median_error']:<10.3f} {result['accuracy_1m']:<8.1f} "
                  f"{result['accuracy_2m']:<8.1f} {result['accuracy_3m']:<8.1f}")
        
        best_result = all_results[0]
        print(f"\n🥇 BEST EXTENDED MODEL: {best_result['model_name']} ({best_result['dataset_size']} samples)")
        ...
        print(f"\n🎬 EXTENDED 200-EPOCH TRAINING COMPLETE!")
        print(f"📁 All extended results saved in: {self.output_dir}")
        
        return all_results, all_histories

def main():
    """Main execution function"""
    system = ExtendedTomCruiseCNNSystem()
    results, histories = system.run_extended_training()
